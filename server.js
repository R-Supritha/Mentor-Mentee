...
const messagesFilePath = path.join(__dirname, "messages.csv");
...
function executeHeapCommand(command, task = '') {
    return new Promise((resolve, reject) => {
        let cmd = `"${tasks}" ${command}`;
        ...
        if (command === 'removeSpecific' && typeof task === 'string') {
          ...
          cmd += ` "${escapedTitle}"`;  // ✅ Add title here
        }
...
app.post('/remove-task', async (req, res) => {
  try {
      ...
      const result = await executeHeapCommand('removeSpecific', title);
      res.send(result);
  } catch (error) {
...
app.get('/current-task', async (req, res) => {
  try {
      const result = await executeHeapCommand('peek');
      if (!result || result.trim() === '') {
          return res.send('No task in the heap currently.');
      }
      res.send(`Current task: ${result}`);
  } catch (error) {
      res.status(500).send('Failed to retrieve current task. Please try again.');
  }
});
...
app.post('/clear-tasks', async (req, res) => {
  try {
      const result = await executeHeapCommand('clear');
      res.send('All tasks cleared from the heap successfully!');
  } catch (error) {
      res.status(500).send('Failed to clear tasks. Please try again.');
  }
});

app.get('/list-tasks', async (req, res) => {
    try {
        const result = await executeHeapCommand('list');
        const tasks = result.trim().split('\n').map(line => {
            const [title, description, dueDate, priority] = line.split('|').map(s => s.trim());
