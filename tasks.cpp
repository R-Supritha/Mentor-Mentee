...
void insertTask(MinHeap* heap, const char* title, const char* description, const char* due_date, int priority) {
...
void LoadFromCSV(MinHeap* heap, const char* filename) {
    ...
    char line[MAX_TITLE_LENGTH + MAX_DESC_LENGTH + MAX_DATE_LENGTH + MAX_PRIORITY_LENGTH + 10];
    while (fgets(line, sizeof(line), file)) {
        char title[MAX_TITLE_LENGTH], description[MAX_DESC_LENGTH], due_date[MAX_DATE_LENGTH], priorityStr[MAX_PRIORITY_LENGTH];
        int priority;

        if (sscanf(line, "%[^,],%[^,],%[^,],%s", title, description, due_date, priorityStr) == 4) {
            priority = atoi(priorityStr);
            insertTask(heap, title, description, due_date, priority);
        }
    }
    fclose(file);
}
...
void SaveToCSV(MinHeap* heap, const char* filename) {
    ...
    for (int i = 0; i < heap->size; i++) {
        fprintf(file, "%s,%s,%s,%d\n", heap->tasks[i].title, heap->tasks[i].description, heap->tasks[i].due_date, heap->tasks[i].priority);
    }
    fclose(file);
}
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(command, "insert") == 0) {
        if (argc < 6) {
            fprintf(stderr, "Usage: %s insert <title> <description> <due_date> <priority>\n", argv[0]);
            clearHeap(&heap);
            return EXIT_FAILURE;
        }
        int priority = atoi(argv[5]);
        insertTask(&heap, argv[2], argv[3], argv[4], priority);
        printf("Task inserted: %s\n", argv[2]);
    }
	    ...
	    if (argc < 3) {
	        fprintf(stderr, "Usage: %s removeSpecific <title>\n", argv[0]);
	        return EXIT_FAILURE;
	    }
        ...
        if (task) {
            printf(" %s | %s | %s | %s\n", task->title, task->description, task->due_date, P);
        } else {
            printf("Heap is empty\n");
        }
    }
	    ...
	    for (int i = 0; i < heap.size; i++) {
	        ...
	        printf("%s | %s | %s | %s\n", heap.tasks[i].title, heap.tasks[i].description, heap.tasks[i].due_date, P);
	    }
	}
