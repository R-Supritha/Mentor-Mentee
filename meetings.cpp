...
bool IsOverlapping(struct meetSlot newMS, struct meetSlot existingMS) {
    return (newMS.slot == existingMS.slot &&
			...
			newMS.booked == existingMS.booked);
}
...
int main(int argc, char* argv[]) {
    ...
    struct meetings* current = front;
    while (current) {
        if (IsOverlapping(newSlot, current->MS)) {
            printf("Error: Time slot already booked\n");
            FreeMeetings(front);
            return 1;
        }
        current = current->next;
